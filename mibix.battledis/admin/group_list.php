...
if($lAdmin->EditAction() && $POST_RIGHT == "W")
{
    ...
    foreach($FIELDS as $ID=>$arFields)
    {
        ...
        $DB->StartTransaction();
        $ID = IntVal($ID);
        $ob = new CMibixDisBattleGroupModel();
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
    ...
    {
        $cData = new CMibixDisBattleGroupModel();
    ...
    foreach($arID as $ID)
    {
        ...
        switch($_REQUEST['action'])
        {
            ...
            case "delete":
                @set_time_limit(0);
                $DB->StartTransaction();
                if(!CMibixDisBattleGroupModel::Delete($ID))
                {
                    $DB->Rollback();
                    $lAdmin->AddGroupError(GetMessage("MIBIX_BATTLEDIS_GROUP_DEL_ERR"), $ID);
                }
                $DB->Commit();
                break;
            ...
            case "deactivate":
                ...
                if(!$ob->Update($ID, $arFields))
                    $lAdmin->AddGroupError(GetMessage("MIBIX_BATTLEDIS_GROUP_SAVE_ERROR").$ob->LAST_ERROR, $ID);
                break;
        }
    }
}
...
$lAdmin->AddHeaders(array(
    ...
    array(
        "id"		=> "name_group",
        "content"	=> GetMessage("MIBIX_BATTLEDIS_POST_F_NAME_GROUP"),
        "sort"		=> "name_group",
        "default"	=> true,
    ),
    array(
        "id"		=> "code_group",
        "content"	=> GetMessage("MIBIX_BATTLEDIS_POST_F_CODE_GROUP"),
        "sort"		=> "code_group",
        "default"	=> true,
    ),
    array(
...
while($arRes = $rsData->NavNext(true, "f_"))
{
    ...
    $row->AddViewField("name_group", '<a href="mibix.battledis_group_edit.php?ID='.$f_id.'&lang='.LANG.'">'.$f_name_group.'</a>');
    ...
    $arActions[] = array(
        "ICON"=>"edit",
        "DEFAULT"=>true,
        "TEXT"=>GetMessage("MIBIX_BATTLEDIS_GROUP_UPD"),
    ...
    if ($POST_RIGHT>="W")
    {
        $arActions[] = array(
            "ICON"=>"delete",
            "TEXT"=>GetMessage("MIBIX_BATTLEDIS_GROUP_DEL"),
            "ACTION"=>"if(confirm('".GetMessage("MIBIX_BATTLEDIS_GROUP_DEL_CONF")."')) ".$lAdmin->ActionDoGroup($f_id, "delete")
        );
    }
...
$APPLICATION->SetTitle(GetMessage("MIBIX_BATTLEDIS_GROUP_TITLE"));
...
$oFilter = new CAdminFilter(
    ...
    array(
        ...
        GetMessage("MIBIX_BATTLEDIS_POST_F_CODE_GROUP"),
...
?>
    <form name="find_form" method="get" action="<?=$APPLICATION->GetCurPage();?>">
        ...
        <tr>
            <td><?=GetMessage("MIBIX_BATTLEDIS_POST_F_CODE_GROUP")?>:</td>
