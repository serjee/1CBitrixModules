...
$aTabs = array(
    array("DIV" => "edit1", "TAB" => GetMessage("MIBIX_BATTLEDIS_TAB_GROUP"), "ICON" => "main_user_edit", "TITLE" => GetMessage("MIBIX_BATTLEDIS_TAB_GROUP_TITLE")),
);
...
{
    $group = new CMibixDisBattleGroupModel();
    ...
    if($res)
    {
        ...
        if($apply!="")
            LocalRedirect("/bitrix/admin/mibix.battledis_group_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
        else
            LocalRedirect("/bitrix/admin/mibix.battledis_group_list.php?lang=".LANG);
    }
    else
    {
        ...
        {
            $message = new CAdminMessage(GetMessage("MIBIX_BATTLEDIS_GROUP_SAVE_ERROR"), $e);
        }
        $bVarsFromForm = true;
    }
}
...
if($ID > 0)
{
    $group = CMibixDisBattleGroupModel::GetByID($ID);
...
if($bVarsFromForm)
{
    $DB->InitTableVarsForEdit("b_mibix_disbattle_group", "", "str_");
}

// Устанавливаем заголовок в зависимости от ее типа (обновление/добавление)
$APPLICATION->SetTitle(($ID > 0 ? GetMessage("MIBIX_BATTLEDIS_GROUP_EDIT_TITLE").$ID : GetMessage("MIBIX_BATTLEDIS_GROUP_ADD_TITLE")));
...
$aMenu = array(
    array(
        "TEXT"=>GetMessage("MIBIX_BATTLEDIS_GROUP_LIST_TEXT"),
        "TITLE"=>GetMessage("MIBIX_BATTLEDIS_GROUP_LIST"),
        "LINK"=>"mibix.battledis_group_list.php?lang=".LANG,
        "ICON"=>"btn_list",
    )
);
...
if($ID>0)
{
    ...
    $aMenu[] = array(
        "TEXT"=>GetMessage("MIBIX_BATTLEDIS_GROUP_ADD_TEXT"),
        "TITLE"=>GetMessage("MIBIX_BATTLEDIS_GROUP_MNU_ADD"),
        "LINK"=>"mibix.battledis_group_edit.php?lang=".LANG,
        "ICON"=>"btn_new",
    );
    $aMenu[] = array(
        "TEXT"=>GetMessage("MIBIX_BATTLEDIS_GROUP_DEL_TEXT"),
        "TITLE"=>GetMessage("MIBIX_BATTLEDIS_GROUP_MNU_DEL"),
        "LINK"=>"javascript:if(confirm('".GetMessage("MIBIX_BATTLEDIS_GROUP_MNU_DEL_CONF")."'))window.location='mibix.battledis_group_list.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."';",
        "ICON"=>"btn_delete",
    );
}
...
if($_REQUEST["mess"] == "ok" && $ID>0)
    CAdminMessage::ShowMessage(array("MESSAGE"=>GetMessage("MIBIX_BATTLEDIS_GROUP_SAVED"), "TYPE"=>"OK"));
if($message)
    echo $message->Show();
?>
    ...
    <form method="POST" action="<?=$APPLICATION->GetCurPage();?>"  enctype="multipart/form-data" name="groupform">
        ...
        ?>
        <tr class="heading">
            <td colspan="2"><?=GetMessage("MIBIX_BATTLEDIS_GROUP_TITLE")?></td>
        </tr>
        <?if($ID > 0):?>
            <tr>
                <td width="40%" class="adm-detail-content-cell-l"><?=GetMessage("MIBIX_BATTLEDIS_GROUP_DATE_ADD");?>:</td>
                <td width="60%" class="adm-detail-content-cell-r"><?=$str_date_insert;?></td>
            </tr>
            <?if($str_date_update <> ""):?>
                <tr>
                    <td width="40%" class="adm-detail-content-cell-l"><?=GetMessage("MIBIX_BATTLEDIS_GROUP_DATE_UPD");?>:</td>
                    <td width="60%" class="adm-detail-content-cell-r"><?=$str_date_update;?></td>
                </tr>
            <?endif?>
        <?endif?>
        <tr>
            <td width="40%" class="adm-detail-content-cell-l"><?=GetMessage("MIBIX_BATTLEDIS_GROUP_ACTIVE");?>:</td>
        ...
        <tr>
            <td width="40%" class="adm-detail-valign-top adm-detail-content-cell-l"><span class="required">*</span>
                <span class="adm-required-field"><?=GetMessage("MIBIX_BATTLEDIS_GROUP_NAME")?></span>:<br>(<?=GetMessage("MIBIX_BATTLEDIS_GROUP_NAME_NOTE")?>)
            </td>
            <td width="60%">
        ...
        <tr>
            <td width="40%" class="adm-detail-valign-top adm-detail-content-cell-l"><span class="required">*</span>
                <span class="adm-required-field"><?=GetMessage("MIBIX_BATTLEDIS_GROUP_CODE")?></span>:<br>(<?=GetMessage("MIBIX_BATTLEDIS_GROUP_CODE_NOTE")?>)
            </td>
            <td width="60%">
        ...
        $tabControl->Buttons(
            array(
                ...
                "back_url"=>"mibix.battledis_group_list.php?lang=".LANG
            )
        );
        ?>
        <?=bitrix_sessid_post();?>
