...
if($RIGHT >= "R") :
    ...
    $arAllOptions = Array(
        array("MO_MIN_PRICE", GetMessage("MIBIX_MO_MIN_PRICE"), array("text", 6)),
        array("MO_MIN_COUNT_ITEM_TYPE", GetMessage("MIBIX_MO_MIN_COUNT_ITEM_TYPE"), array("text", 6)),
        array("MO_MIN_COUNT_ITEM", GetMessage("MIBIX_MO_MIN_COUNT_ITEM"), array("text", 6)),
    );
    ...
    CModule::IncludeModule(str_replace(".","_",$MODULE_ID));
    ...
    if($REQUEST_METHOD=="POST" && strlen($Update.$Apply.$RestoreDefaults) > 0 && $RIGHT=="W" && check_bitrix_sessid())
    {
        ...
        else
        {
            foreach($arAllOptions as $arOption)
            {
                ...
                COption::SetOptionInt($MODULE_ID, $name, $val, $arOption[1]);
            }
        }
    ...
    $tabControl->BeginNextTab();
        foreach($arAllOptions as $arOption):
            $val = COption::GetOptionInt($MODULE_ID, $arOption[0]);
            $type = $arOption[2];
            ?>
            <tr>
                ...
                <td width="60%">
                    ...
                    <?elseif($type[0]=="text"):?>
                        <input type="text" size="<?echo $type[1]?>" maxlength="255" value="<?echo htmlspecialcharsbx($val)?>" name="<?echo htmlspecialcharsbx($arOption[0])?>" id="<?echo htmlspecialcharsbx($arOption[0])?>"> <?=GetMessage("MIBIX_COMMENT_IF_ZERO")?>
