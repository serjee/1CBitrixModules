...
if($lAdmin->EditAction() && $POST_RIGHT == "W")
{
    ...
    foreach($FIELDS as $ID=>$arFields)
    {
        ...
        $DB->StartTransaction();
        $ID = IntVal($ID);
        $ob = new CMibixBattleBattleModel();
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
    ...
    {
        $cData = new CMibixBattleBattleModel();
    ...
    foreach($arID as $ID)
    {
        ...
        switch($_REQUEST['action'])
        {
            ...
            case "delete":
                @set_time_limit(0);
                $DB->StartTransaction();
                if(!CMibixBattleBattleModel::Delete($ID))
                {
                    $DB->Rollback();
                    $lAdmin->AddGroupError(GetMessage("MIBIX_BATTLE_BATTLE_DEL_ERR"), $ID);
                }
                $DB->Commit();
                break;
            ...
            case "deactivate":
                ...
                if(!$ob->Update($ID, $arFields))
                    $lAdmin->AddGroupError(GetMessage("MIBIX_BATTLE_BATTLE_SAVE_ERROR").$ob->LAST_ERROR, $ID);
                break;
        }
    }
}
...
$lAdmin->AddHeaders(array(
    ...
    array(
        "id"		=> "name_battle",
        "content"	=> GetMessage("MIBIX_BATTLE_NAME_BATTLE"),
        "sort"		=> "name_battle",
        "default"	=> true,
    ),
    array(
        "id"		=> "name_group",
        "content"	=> GetMessage("MIBIX_BATTLE_NAME_GROUP"),
        "sort"		=> "name_group",
        "default"	=> true,
    ),
    array(
...
while($arRes = $rsData->NavNext(true, "f_"))
{
    ...
    $row->AddViewField("name_battle", '<a href="mibix.battle_battle_edit.php?ID='.$f_id.'&lang='.LANG.'">'.$f_name_battle.'</a>');
    ...
    $arActions[] = array(
        "ICON"=>"edit",
        "DEFAULT"=>true,
        "TEXT"=>GetMessage("MIBIX_BATTLE_BATTLE_UPD"),
    ...
    if ($POST_RIGHT>="W")
    {
        $arActions[] = array(
            "ICON"=>"delete",
            "TEXT"=>GetMessage("MIBIX_BATTLE_BATTLE_DEL"),
            "ACTION"=>"if(confirm('".GetMessage("MIBIX_BATTLE_BATTLE_DEL_CONF")."')) ".$lAdmin->ActionDoGroup($f_id, "delete")
        );
    }
...
$APPLICATION->SetTitle(GetMessage("MIBIX_BATTLE_BATTLE_TITLE"));
...
$oFilter = new CAdminFilter(
    ...
    array(
        ...
        GetMessage("MIBIX_BATTLE_POST_F_NAME_BATTLE"),
...
?>
    <form name="find_form" method="get" action="<?=$APPLICATION->GetCurPage();?>">
        ...
        <tr>
            <td><?=GetMessage("MIBIX_BATTLE_POST_F_NAME_BATTLE")?>:</td>
